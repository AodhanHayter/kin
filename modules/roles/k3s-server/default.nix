...
with kin;
let
  cfg = config.kin.roles.k3s-server;
in
{
  imports = [
    ../../system/k3s-base
  ...
  options.kin.roles.k3s-server = {
    enable = mkBoolOpt false "Run this node as the k3s control-plane (server) that inits the cluster.";
  };

  config = mkIf cfg.enable {
    # The server is the one node that ships cluster-wide observability.
    kin.services.monitoring = enabled;
