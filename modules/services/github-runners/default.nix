# kin/github-runners — GitHub Actions runners via ARC (actions-runner-
# controller), deployed from the server through k3s' helm-controller. One runner
# scale set per entry in `repos`; workflows target them with
# `runs-on: homelab-k3s`. Runners are ephemeral, scale 0->4, dind for docker.
# Single role, applied to the k3s-server (atlas) where the PAT is consumed.
#
...
{
  _class = "clan.service";
  manifest.name = "kin/github-runners";
  manifest.description = "ARC GitHub Actions runner scale sets, deployed into k3s.";
  manifest.categories = [ "Development" ];

  roles.default = {
    description = "Deploy ARC runner scale sets from this server.";
    perInstance =
      { ... }:
      {
        nixosModule =
          ...
          in
          {
            # PAT — consumed only here (server), so scoped to the instance.
            ...
            # The PAT can't live in a HelmChart manifest (nix store is
            # world-readable), so it is synced from the sops-decrypted var into a
            # k8s Secret at boot.
