# Cluster observability: kube-prometheus-stack (Prometheus + Grafana +
...
# same pattern as Longhorn. Server-side module: enabled by the k3s-server role.
{
  config,
  lib,
  pkgs,
...
with kin;
let
  cfg = config.kin.services.monitoring;
in
{
  options.kin.services.monitoring = {
    enable = mkBoolOpt false "Whether to deploy kube-prometheus-stack from this server.";
  };

  config = mkMerge [
    # Grafana admin password generator — defined unconditionally so the
    # encrypted var never depends on the toggle.
    {
    ...
    (mkIf cfg.enable {
      ...
      # network. etcd gets its own flag (serves http on 0.0.0.0:2381).
