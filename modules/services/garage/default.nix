...
with kin;
let
  cfg = config.kin.services.garage;
...
in
{
  ...
  options.kin.services.garage = {
    enable = mkBoolOpt false "Whether to run a Garage S3 node (backup target).";
  };

  config = mkMerge [
    # RPC secret generator — defined unconditionally so the encrypted var
